...
def calculate_rho_tilde(rho: np.ndarray, k: int) -> np.ndarray:
    ...
    sum_c_1d = 1.0 + (k - 1) * 1.0
    w = sum_c_1d * sum_c_1d  # W = k^2

    for i in range(k, N, k):  # i = y-index
        for j in range(k, N, k):  # j = x-index
            weighted_sum = 0.0
            ...
            for alpha_idx in range(-half_k, half_k + 1):
                ...
                for beta_idx in range(-half_k, half_k + 1):
                    ...
                    i_prime = i + alpha_idx  # y'
                    j_prime = j + beta_idx  # x'

                    if 0 <= i_prime <= N and 0 <= j_prime <= N:
                        # Indeksowanie: rho[y, x]
...
def calculate_S(V: np.ndarray, rho_tilde: np.ndarray, k: int) -> float:
    ...
    for i in range(0, N - k, k):  # i = y-index (row)
        for j in range(0, N - k, k):  # j = x-index (column)

            # dV/dx_avg (zmiana w j - x)
            dV_dx_avg = (
                                (V[i, j + k] - V[i, j]) +
                                (V[i + k, j + k] - V[i + k, j])
                        ) * denom_factor

            # POPRAWIONY dV/dy_avg (zmiana w i - y)
            dV_dy_avg = (
                                (V[i + k, j] - V[i, j]) +
                                (V[i + k, j + k] - V[i, j + k])
                        ) * denom_factor
...
@jit(nopython=True)
def numba_iter_red_black(V: np.ndarray, rho_tilde: np.ndarray, k: int):
    global D_x, N
    factor = (k * D_x)

    # Krok 1: Relaksacja Czerwonych węzłów
    for i in range(k, N, k):
        for j in range(k, N, k):
            # Warunek Czerwony: (i/k + j/k) musi być parzyste
            if ((i // k) + (j // k)) % 2 == 0:
                Vs = (
                        V[i - k, j] +
                        V[i + k, j] +
                ...
                V[i, j] = 0.25 * (Vs + factor ** 2 * rho_tilde[i, j])

    # Krok 2: Relaksacja Czarnych węzłów
    for i in range(k, N, k):
        for j in range(k, N, k):
            # Warunek Czarny: (i/k + j/k) musi być nieparzyste
            if ((i // k) + (j // k)) % 2 != 0:
                Vs = (
                        V[i - k, j] +
                        V[i + k, j] +
...
def relax_local(V: np.ndarray, k: int):
    ...
    for it in range(MAX_ITER):
        # Użycie schematu Red-Black
        numba_iter_red_black(V, rho_tilde, k)
...
for idx, k in enumerate([16, 8, 4, 2, 1]):
    ...
    # Przygotowanie do wyświetlania mapy
    y_idx = np.arange(0, N + 1, k)
    x_idx = np.arange(0, N + 1, k)
    V_lowres = V[np.ix_(y_idx, x_idx)]

    ax = axs[idx]
    ax.set_title(f"k={k}, iter: {len(sums)}")
    im = ax.imshow(V_lowres, origin='lower')
...
ax = axs[5]
ax.set_title(f"Całka S w danej iteracji")
ax.legend()
plt.show()